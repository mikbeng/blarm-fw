...
RTC_DATA_ATTR sleep_handle_t sleep_handle = {0};
...
void pwr_init(void)
{
    //Configure pwr_off_pin as output
    ...
    //Configure bat_mon_pin as input
    ...
    if (wakeup_cause == ESP_SLEEP_WAKEUP_GPIO) { 
        sleep_handle.wake_count++;
        // Get and store which pins triggered the wakeup
        sleep_handle.last_wakeup_pins = esp_sleep_get_gpio_wakeup_status();
        
        ESP_LOGI(TAG, "Woke up from deep sleep!");
        ESP_LOGI(TAG, "Wake count: %ld", sleep_handle.wake_count);
        
        if (sleep_handle.last_wakeup_pins & (1ULL << WAKEUP_PIN)) {
...
void pwr_enter_deep_sleep(uint32_t state_to_save)
{
    ...
    sleep_handle.last_state = state_to_save;
    sleep_handle.wake_count++;
...
{
    return sleep_handle.wake_count;
}

uint32_t pwr_get_last_state(void)
{
    return sleep_handle.last_state;
}

uint64_t pwr_get_last_wakeup_pins(void)
{
    return sleep_handle.last_wakeup_pins;
}
